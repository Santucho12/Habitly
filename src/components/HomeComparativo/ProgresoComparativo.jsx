      ...
      {/* Card Usuario */}
        <div className="rounded-3xl border-4 border-blue-900 bg-gradient-to-br from-gray-900 via-blue-900 to-gray-800 shadow-2xl p-6 flex flex-col items-center justify-center text-white w-64" style={{ boxShadow: '0 8px 32px 0 rgba(0,0,0,0.5)', transform: 'scale(0.6)', transformOrigin: 'top left', marginTop: '-120px' }}>
          ...
          <div className="mb-3">
            ...
            <span className="font-bold text-2xl text-white drop-shadow">{usuario?.pesoActual ?? '-'}</span> <span className="text-base">kg</span>
          </div>
          <div className="mb-3 flex items-center gap-2">
            <span className="text-base font-semibold text-cyan-100">Kilos bajados:</span>
            <FaArrowDown className="text-cyan-200 text-2xl" />
            <span className="font-bold text-2xl text-white drop-shadow">{usuario?.kilosBajados ?? '-'}</span> <span className="text-base">kg</span>
          </div>
          <span className="text-sm text-cyan-100 mt-2 font-medium">{usuario?.mes || ''}</span>
        </div>
      {/* Card Compañero */}
        <div className="rounded-3xl border-4 border-pink-900 bg-gradient-to-br from-gray-900 via-pink-900 to-gray-800 shadow-2xl p-6 flex flex-col items-center justify-center text-white w-64" style={{ boxShadow: '0 8px 32px 0 rgba(0,0,0,0.5)', transform: 'scale(0.6)', transformOrigin: 'top left', marginTop: '-120px', marginLeft: '-86px' }}>
          ...
          <div className="mb-3">
            ...
            <span className="font-bold text-2xl text-white drop-shadow">{companero?.pesoActual ?? '-'}</span> <span className="text-base">kg</span>
          </div>
          <div className="mb-3 flex items-center gap-2">
            <span className="text-base font-semibold text-pink-100">Kilos bajados:</span>
            <FaArrowDown className="text-pink-200 text-2xl" />
            <span className="font-bold text-2xl text-white drop-shadow">{companero?.kilosBajados ?? '-'}</span> <span className="text-base">kg</span>
          </div>
          <span className="text-sm text-pink-100 mt-2 font-medium">{companero?.mes || ''}</span>
        </div>
    </div>
  );
}
